...
The estimate values of 1.004 for chlorophyll and 0.998 for sla show that there is highly linear relationship between total mass and these two variables per species. The low values for standard error for both .001 show that there is not a large deviance from the estimates to the real data which further shows strong prediction. The respect statistic value's deviance from one for chlorophyll and sla (3.372 and -1.923) also shows strong prediction. 
...
plot(ggpredict(full_log, terms = "chlorophyll", back.transform = TRUE), add.data = TRUE) + labs(caption ="Chlorophyll as a Predictor Variable for Biomass") + theme(plot.caption = element_text(hjust = 0.2))
